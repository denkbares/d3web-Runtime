...
public class VertexColorTransformer implements Transformer<RatedTestCase, Paint> {
	...
	/**
	 * Returns a color depending on the information given in a RatedTestCase.
	 * 
	 * @param rtc RatedTestCase
	 * 
	 * @return GREEN, if RatedTestCase.isCorrect(), RED, if
	...
	public Paint transform(RatedTestCase rtc) {
		...
		// String color =
		// ConfigLoader.getInstance().getProperty("edgeColorNewCase");
		//    	
		// if (!rtc.isCorrect()) {
		// color =
		// ConfigLoader.getInstance().getProperty("edgeColorIncorrectCase");
