...
public class Config {
	...
	static {
		baseConfig = ConfigReader.createConfig(ConfigReader.class.getClassLoader().getResource(
				"config_base.xml"));
	}
	...
	public String getClass(String key) {
		if (this.classes.containsKey(key)) return this.classes.get(key);
		return null;
	}
	...
	public String getConverter(String key) {
		if (this.converters.containsKey(key)) return this.converters.get(key);
		return null;
	}
	...
	private static Config getResultConfig(List<Config> configs, String key) {
		...
		for (Config conf : configs) {
			if (conf.getValue(key) != null) values.add((Config) conf.getValue(key));
		}
		if (values.size() < 2) return values.get(0);
		else return buildResultConfig(values);
	}

	private static boolean valueIsConfig(List<Config> configs, String key) {
		for (Config conf : configs)
			if (conf.getValue(key) instanceof Config) return true;
		return false;
	}
		...
		for (Config config : configs) {
			if (config.getValue(key) != null) {
				if (result.getConverter(key) == null || result.getConverter(key).equals("")) {
					...
					result.setValue(key, config.getValue(key));
					if (!(config.getConverter(key) == null || config.getConverter(key).equals(""))) result.setConverter(
							key, config.getConverter(key));
				}
			}
		}
	}
