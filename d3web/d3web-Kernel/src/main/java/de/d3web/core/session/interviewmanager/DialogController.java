...
public interface DialogController extends QASetManager {
	...
	/**
	 * @return true, if there is any QASet that can be answered in the current case
	 */
	...
	/** moves to the next QASet (e.g. on history) even if it has already been answered
	 * 
	 * @return next QASet
	 */
	...
	/**
	 * Returns the next remaining QASet (e.g. on qaSetQueue) even if the current QASet
	 * hasn't been answered completely.
	 * The currentQASet, newestQASet and nextQASet will not be changed by this method.
	 * Multiple method-calls are iterating the nextQASet's, until "moveToNewestQASet"
	 * is called (then, the nextRemainingQASet is the QASet that comes after the
	 * currentQASet).	 * @return QASet
	...
	/**
	 * jumps to Question q or to QContainer that is parent of q (depending on DialogController type)
	 * @return the QASet the controller has jumped to
	 */
