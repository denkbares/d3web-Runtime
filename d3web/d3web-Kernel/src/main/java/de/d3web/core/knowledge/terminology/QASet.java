...
public abstract class QASet extends NamedObject implements InterviewObject, CaseObjectSource {
	...
	/**
	 * Activates a question. That means it will be added to the list of
	 * questions to ask if it was not asked yet
	 * 
	...
	public void activate(Session theCase, Rule rule, Class<? extends PSMethod> psm) {
		...
		if (!isDone(theCase)) {
			if (caseQA.getContraReasons().contains(source)) {
				...
				if (!caseQA.hasContraReason()) {
					theCase.getQASetManager().propagate(this, rule,
							theCase.getPSMethodInstance(psm));
		...
		else if ((source.getRule() != null)
				&& (source.getRule().getAction() instanceof ActionNextQASet)) {
			if (!caseQA.getProReasons().contains(source)) caseQA.addProReason(source);
		}
	}
	...
	/**
	 * removes "source" from the list of pro reasons, if the question has been
	 * activated from a pro reason Otherwise "source" will be added to contra
	 * reason list
	 */
