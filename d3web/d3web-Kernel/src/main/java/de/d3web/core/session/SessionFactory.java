...
public class SessionFactory {
	...
	// if ((registeredQContainers != null) && (registeredQContainers.size() >
	// 0)) {
	// for (QASet qaSet : registeredQContainers) {
	// List<?> pros = qaSet.getProReasons(newCase);
	// if ((pros == null) || (pros.size() == 0)) {
	// qaSet.addProReason(new QASet.Reason(null, PSMethodUserSelected.class),
	// newCase);
	// }
	...
	// || ((qaSet instanceof QContainer) &&
	// (mqdc.nothingIsDoneInContainer((QContainer) qaSet)))) {
	...
	// * given proxy in an order that is defined by the given
	// MQDialogController.
	// *
	// * @param qaSet (QASet the container whose question-children are to
	// answer)
	...
	// * answer)
	// */
	// private static void answerQuestions(QASet qaSet, DialogProxy proxy,
	// Session inCase, MQDialogController mqdc) {
	...
	// List<Question> validQuestions = mqdc.getAllValidQuestionsOf((QContainer)
	// qaSet);
	...
	// * indicated questions are answered with the values of "proxy" (if any).
	// The
	...
	// DialogProxy proxy,
	// List<? extends PSMethod> usedPSMethods) {
	// return createAnsweredSession(kb, dialogControllerType, proxy, null,
	// usedPSMethods);
	// }

	// private static class QASetManagerFactoryAdapter implements
	// QASetManagerFactory {
	//
	// private Class<? extends QASetManager> qaSetManagerClass = null;
	//
	// public QASetManagerFactoryAdapter(Class<? extends QASetManager>
	// qaSetManagerClass) {
	// super();
