...
public class NumInConditionHandler implements FragmentHandler {
		...
		if (questionID!=null) {
			...
			if (idObject instanceof QuestionNum) {
				...
				if (item != null && childNodes.size()== 1) {
					NumericalInterval interval = (NumericalInterval) PersistenceManager.getInstance().readFragment(item, kb);
	...
	public Element write(Object object, Document doc) throws IOException {
		...
		Element intervalElemenent = PersistenceManager.getInstance().writeFragment(cond.getInterval(), doc);
		Element element = XMLUtil.writeConditionWithValueNode(doc, cond.getQuestion(), "numIn", intervalElemenent);;
		return element;
	}
