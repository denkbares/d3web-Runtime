...
/**
 * Introducing a utility class for more simplified logging. This class allows to
 * log by a class instance or an instance of any object without accessing the
 * class's name. It also allows to log exceptions with the handy "fine", "info",
 * "warn" or "severe" method to minimize logging code.
 * <p>
 * There are also methods to do logging without even specifying the callers
 * class or instance. In this case the log methods will retract the relevant
 * class from the stack trace automatically.
 * 
...
public class Log {
	...
	/**
	 * Enumeration to access the different {@link StackFrameFactory}s. Please
	 * note that there is no link dependency between this log class and the
	 * particular StackFrameFactory implementations. This is required, because
	 * depending on the JVM loading a class may fail if the linked class if not
	 * present. Thus this class can only load the implementations by
	 * reflections.
	 * <p>
	 * The enumeration is ordered by the preference of the implementations (in
	 * terms of speed). The last one is an implementation that will work in
	 * every environment. Therefore there will always be an instance available
	 ...
	 * <p>
	 * Please note that the stack level specified for the methods must be in
	 * alignment with the stackLevel used for the factory to create the specific
	 * stack frame.
	 * <p>
	 * In alignment means: counting backward with the specified stackLevel at
	 * calling the stack frame factory must point to the same stack frame that
	 * counting backward with the specified stackLevel when calling the methods
	 * of this instance.
	 * 
	 * @author Volker Belli (denkbares GmbH)
	 * @created 19.01.2014
	...
	interface StackFrame {
		 ...
		 * <p>
		 * Please note that the stack level specified for the methods must be in
		 * alignment with the stackLevel used for the factory to create the
		 * specific stack frame.
		 * <p>
		 * In alignment means: counting backward with the specified stackLevel
		 * at calling the stack frame factory must point to the same stack frame
		 * that counting backward with the specified stackLevel when calling the
		 * methods of this instance.
		 * 
		 * @created 19.01.2014
		 * @param stackLevel the stack level to be in alignment to this stack
		 *        frame
	...
	/**
	 * We require some interface to extract the caller, because the default
	 * implementation may not work on non-sun-java environments. In this case we
	 * drop down to some other behavior.
	 * 
	 * @author Volker Belli (denkbares GmbH)
	 * @created 19.01.2014
	 */
	interface StackFrameFactory {
		...
		/**
		 * Returns the callers class and method name of the specified call
		 * before, a positive number of call stack frames before. If stackLevel
		 * is specified as "0", the method and class name of the caller of this
		 * method is returned.
		 * 
		 * @created 19.01.2014
		 * @param stackLevel the stack level, starting to count at the caller of
		 *        this method
	...
	/**
	 * Class name extraction from the jvm native class. This is the most
	 * imcompatible method, but also the fastest one.
	 * 
	 * @author Volker Belli (denkbares GmbH)
	 * @created 19.01.2014
	 */
	...
	/**
	 * Class name extraction based on an exception stack trace. This is a very
	 * compatible method, but also a slow one.
	 * 
	 * @author Volker Belli (denkbares GmbH)
	 * @created 19.01.2014
	 */
	...
	/**
	 * Implementation to ignore class name extraction. Very quick and
	 * compatible, but also not that flexible.
	 * 
	 * @author Volker Belli (denkbares GmbH)
	 * @created 19.01.2014
	 */
	...
	/**
	 * Initializes this logger with a special method to detect the source class
	 * for logging the specific messages later on. You may not call this method
	 * manually, because it will be automatically initialized as it is needed.
	 * Only if you prefer a special logger detection method you may call this
	 * method, e.g. for special runtime environments.
	 * 
	 * @created 19.01.2014
	 * @param method the method of class detection to be used for logging
	 * @throws IllegalStateException if this method will not been supported by
	 *         the currently used virtual machine implementation
	 */
	...
	/**
	 * Log a FINEST message, together with the throwable that is responsible to
	 * cause this message.
	 * <p>
	 * If the logger is currently enabled for the FINEST message level then the
	 * given message is forwarded to all the registered output Handler objects.
	...
	/**
	 * Log a FINER message, together with the throwable that is responsible to
	 * cause this message.
	 * <p>
	 * If the logger is currently enabled for the FINER message level then the
	 * given message is forwarded to all the registered output Handler objects.
	...
	/**
	 * Log a FINE message, together with the throwable that is responsible to
	 * cause this message.
	 * <p>
	 * If the logger is currently enabled for the FINE message level then the
	 * given message is forwarded to all the registered output Handler objects.
	...
	/**
	 * Log a INFO message, together with the throwable that is responsible to
	 * cause this message.
	 * <p>
	 * If the logger is currently enabled for the INFO message level then the
	 * given message is forwarded to all the registered output Handler objects.
	...
	/**
	 * Log a WARNING message, together with the throwable that is responsible to
	 * cause this message.
	 * <p>
	 * If the logger is currently enabled for the WARNING message level then the
	 * given message is forwarded to all the registered output Handler objects.
	...
	/**
	 * Log a SEVERE message, together with the throwable that is responsible to
	 * cause this message.
	 * <p>
	 * If the logger is currently enabled for the SEVERE message level then the
	 * given message is forwarded to all the registered output Handler objects.
	...
	/**
	 * Returns the {@link Logger} from java logging used to log any messages of
	 * the callers class. Use this method if the specified utility methods are
	 * not sufficient for your special purpose.
	 * 
	 * @created 19.01.2014
