...
public class DialogTester {
	...
	public void setUp() throws Exception {
		...
		ask_for_pregnancy = kbm.createQuestionOC("ask for pregnancy", pregnancyQuestions, new String[] {"yes", "no"});
		...
		initQuestion = kbm.createQuestionOC("initQuestion", root, new String[] {"all","pregnacyQuestions","height+weight"});
		
		pregnancyContainerIndication = kbm.createQuestionOC("pregnancyContainerIndication", root, new String[] {"yes", "no"});
		...
		// Rule: initQuestion = pregnacyQuestions => INDICATE CONTAINER ( pregnancyQuestions ) 
		...
		// Rule: initQuestion = height+weight => INDICATE CONTAINER ( heightWeightQuestions ) 
		RuleFactory.createIndicationRule("r4", heightWeightQuestions, 
				new CondEqual(initQuestion, 
						new ChoiceValue(kbm.findChoice(initQuestion, "height+weight"))));

		// Rule: initQuestion = all => INDICATE CONTAINER ( pregnancyQuestions, heightWeightQuestions ) 
		RuleFactory.createIndicationRule("r5", Arrays.asList(new QASet[] {pregnancyQuestions, heightWeightQuestions}), 
				new CondEqual(initQuestion, new ChoiceValue(kbm.findChoice(initQuestion, "all"))));

		// Rule: pregnancyContainerIndication = yes => INDICATE CONTAINER ( pregnancyQuestions ) 
	...
	public void testQContainerIndication() {		
		...
		// EXPECT: INDICATE CONTAINER ( pregnancyQuestions, heightWeightQuestions )
	...
	public void testQContainerIndicationByMutlipleRules() {		
		...
		// EXPECT: INDICATE CONTAINER ( pregnancyQuestions )  // i.e. doubled indication of pregnancyQuestions
		setValue(pregnancyContainerIndication, new ChoiceValue(kbm.findChoice(pregnancyContainerIndication, "yes")));
		assertEquals(
				new Indication(State.INDICATED), 
				session.getBlackboard().getIndication(pregnancyQuestions));
		...
		//         INDICATE CONTAINER ( pregnancyQuestions )  // due to doubled indication  
		setValue(initQuestion, UndefinedValue.getInstance());
		assertEquals(
				new Indication(State.INDICATED), 
				session.getBlackboard().getIndication(pregnancyQuestions));
