 ...
 * of abstract multiplechoice questions' values
 * 
 ...
 * 
 * Day = Workday => Weekday = Monday, Tuesday, Wednesday, Thursday, Friday
 * Day = Weekend => Weekday = Saturday, Sunday
 * Day = Workday, Weekend => Weekday = Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday,
 * 
 * 
...
public class MultipleChoiceAbstractionTest {
	...
	private static void addTerminologyObjects() {
		...
		Question weekdayQuestion = 
			kbm.createQuestionMC(weekday, kbm.getKnowledgeBase().getRootQASet(), weekdayAlternatives);
	...
	private static void addRules() {
		...
		RuleFactory.createSetValueRule(kbm.createRuleID(), weekday, getWeekdayMCValue(ignoredValues), condition);
		...
		RuleFactory.createSetValueRule(kbm.createRuleID(), weekday, getWeekdayMCValue(null), condition);
	}
	
	/**
	 * Returns a MultipleChoiceValue containing all Weekdays which are not
	 * in the committed List of ignored days.
	 * @param ignoredValues the days which will NOT be added to the MultipleChoiceValue
	...
	private static MultipleChoiceValue getWeekdayMCValue(List<String> ignoredValues) {
		...
		for (Choice c : weekday.getAllAlternatives()) {
			if (ignoredValues == null || !ignoredValues.contains(c.getName()))
				weekdayValues.add(new ChoiceValue(c));
		}
	...
	public void testAbstractionProperty() {
		...
		Question weekday = kbm.findQuestion("Weekday");
		Boolean abstractionProperty = (Boolean) weekday.getProperties().getProperty(Property.ABSTRACTION_QUESTION);
	...
	public void testSetMultipleChoiceValue() {
		...
		session.setValue(day, getWeekdayMCValue(Arrays.asList("")));
		
		// TEST 'Day' == 'Weekend, Workday'
		Value dayValue = session.getValue(day);
		assertEquals("Question 'Day' has wrong value", getWeekdayMCValue(Arrays.asList("")), dayValue);
		
		// TEST 'Weekday' == 'Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday'
		Value weekdayValue = session.getValue(weekday);
		MultipleChoiceValue completeWeekValues = getWeekdayMCValue(null);
		assertEquals("Abstract question 'Weekday' has wrong value", completeWeekValues, weekdayValue);
	}
	...
	public void testSetUndefinedValue() {
		...
		// SET 'Day' = 'UNDEFINED'
		session.setValue(day, UndefinedValue.getInstance());
		...
		Value weekdayValue = session.getValue(weekday);
		assertEquals("Abstract question 'Day' has wrong value", UndefinedValue.getInstance(), weekdayValue);
	}
