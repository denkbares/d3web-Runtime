...
public class TextAbstractionTest {
	...
	private static void addTerminologyObjects() {
		...
		String[] eventAlternatives = {"Happiness", "Sadness"};
		Question event = kbm.createQuestionOC("Feeling", kbm.getKnowledgeBase().getRootQASet(), eventAlternatives);
	...
	public void testTerminlogyObjectExistence() {
		...
		// Values of 'Feeling'
		Value happiness = kbm.findValue(feeling, "Happiness");
		assertNotNull("Value 'Happiness' of Question 'Feeling' isn't in the Knowledgebase", happiness);
		Value sadness = kbm.findValue(feeling, "Sadness");
	...
	public void testAbstractionProperty() {
		...
		// Feeling <abstract> ?
		Question feeling = kbm.findQuestion("Feeling");
		Boolean feelingAbstractionProperty = (Boolean) feeling.getProperties().getProperty(Property.ABSTRACTION_QUESTION);
	...
	public void testSetAndChangeValue() {
		...
		session.setValue(emoticon, happinessIcon);
		...
		session.setValue(emoticon, sadnessIcon);
	...
	public void testSetUndefinedValue() {
		...
		session.setValue(emoticon, UndefinedValue.getInstance());
		
		// TEST 'Emoticon' == 'UNDEFINED'
		Value currentEmoticonValue = session.getValue(emoticon);
		assertEquals("Question 'Emoticon' has wrong value", UndefinedValue.getInstance(), currentEmoticonValue);
		
		// TEST 'Feeling' == 'UNDEFINED'
		Value currentFeelingValue = session.getValue(feeling);
		assertEquals("Question 'Feeling' has wrong value", UndefinedValue.getInstance(), currentFeelingValue);
