...
public class FormulaExpressionAbstractionTest {
	...
	private static void addTerminologyObjects() {
		...
		// Question 'Category'
		String[] categoryAlternatives = {"Underweight", "Normal", "Overweight"};
		Question category = kbm.createQuestionOC("Category", kbm.getKnowledgeBase().getRootQASet(), categoryAlternatives);
	...
	private static void addRules() {
		...
		RuleFactory.createSetValueRule(kbm.createRuleID(), category, underweight, underweightCondition);
		...
		RuleFactory.createSetValueRule(kbm.createRuleID(), category, overweight, overweightCondition);
	}
	...
	public void testTerminlogyObjectExistence() {
		...
		// Values of 'Category'
		Value underweight = kbm.findValue(category, "Underweight");
		assertNotNull("Value 'Underweight' of Question 'Category' isn't in the Knowledgebase", underweight);
		Value normal = kbm.findValue(category, "Normal");
		assertNotNull("Value 'Normal' of Question 'Category' isn't in the Knowledgebase", normal);
		Value overweight = kbm.findValue(category, "Overweight");
		assertNotNull("Value 'Overweight' of Question 'Category' isn't in the Knowledgebase", overweight);
	}
	...
	public void testAbstractionProperty() {
		...
		// TEST BMI <abstract> ?
		Question bmi = kbm.findQuestion("BMI");
		Boolean bmiAbstractionProperty = (Boolean) bmi.getProperties().getProperty(Property.ABSTRACTION_QUESTION);
		assertEquals("Question 'BMI' isn't abstract.", Boolean.TRUE, bmiAbstractionProperty);
		
		// TEST Category <abstract> ?
		Question category = kbm.findQuestion("Category");
		Boolean categoryAbstractionProperty = (Boolean) category.getProperties().getProperty(Property.ABSTRACTION_QUESTION);
		assertEquals("Question 'Category' isn't abstract.", Boolean.TRUE, categoryAbstractionProperty);
	}
	...
	public void testSetValues() {
		...
		assertEquals("Abstract Question 'BMI' has wrong value", bmiValue, currentBmiValue.getValue());
	...
	public void testSetUndefinedValue() {
		...
		assertEquals("Abstract Question 'BMI' has wrong value", UndefinedValue.getInstance(), bmiValue);
		
		// TEST 'Category' == 'UNDEFINED' 
		Value categoryValue = session.getBlackboard().getValue(category);
		assertEquals("Abstract Question 'Category' has wrong value", UndefinedValue.getInstance(), categoryValue);
	}
