...
public class ENode {
	...
	private NamedObject target = null; // e.g. the question or diagnosis to
										// explain
	private TargetValue value = null; // FF wie kann eine Aw-Alternative erklärt
										// werden (in LispD3 nicht möglich!)
	private Collection contexts = null;

	private Collection proReasons = null; // Collection of EReasons
	// bei Diagnosis nur "P-Regeln"; bei QContainer und Questions nur
	// Indikationsregeln
	private Collection contraReasons = null; // Collection of EReasons
	// bei Diagnosis nur "N-Regeln"; bei QContainer und Questions nur
	// KontraInd-regeln
	...
	// Iterator pros =
	// ((QASet)
	// getTarget()).getProReasons(getFactory().getSession()).iterator();
	// while (pros.hasNext()) {
	// Object pro = pros.next();
	// if ((pro instanceof QASet.Reason) && (contexts.contains(((QASet.Reason)
	// pro).getProblemSolverContext()))) {
	...
	private Collection getQASetContraReasons() {
		...
		// Iterator cons =
		// ((QASet)
		// getTarget()).getContraReasons(getFactory().getSession()).iterator();
		// while (cons.hasNext()) {
		// Object con = cons.next();
		// if ((con instanceof QASet.Reason) &&
		// (contexts.contains(((QASet.Reason) con).getProblemSolverContext())))
		// {
		// retValues.add(EReason.createReason(getFactory(), (QASet.Reason)
		// con));
		// } else {
		// System.err.println("Ups, kein QASet.Reason. Dürfte eigentlich nicht sein!");
		// }
