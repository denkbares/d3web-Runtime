 ...
 * created TestSuite in KnOffice-Format to a txt-File.
 * 
...
public class TestSuiteKnOfficeWriter extends AbstractTestSuiteWriter {
	...
	private String quoteTextIfRequired(String text) {
		if (needsQuotation(text)) return "\"" + text + "\"";
		else return text;
	}

	private boolean needsQuotation(String text) {
		for (int i = 0; i < BAD_CHARS.length(); i++) {
			if (text.indexOf(BAD_CHARS.charAt(i)) > -1) return true;
		}
		return false;
	}
