...
public class TestCaseComparator {
	...
	private void performFuzzyComparison() {
		...
		for (SequentialTestCase stc : cases1) {
			if (contains(cases2, stc))
				intersectingCases.add(stc);
			else
				onlyInCase1.add(stc);
		}
		for (SequentialTestCase stc : cases2) {
			if (!contains(cases1, stc))
				onlyInCase2.add(stc);
		}
	}

	/**
	 * Checks if the SequentialTestCases contain
	 * the same RatedTestCases
	...
	/**
	 * Returns true if the RatedTestCases
	 * have the same findings and the same
	 * expected solutions
	...
	public static boolean equals(RatedTestCase rtc1, RatedTestCase rtc2) {
		...
		for (RatedSolution rs : rtc1.getExpectedSolutions()) {
			if (!rtc2.getExpectedSolutions().contains(rs))
				return false;
		}
		return true;
	}
	...
	/**
	 * Returns true if two SequentialTestCases have the same
	 * RatedTestCases.
	...
	public static boolean equals(SequentialTestCase stc1,
		...
		if (stc1.getCases().size() != stc2.getCases().size())
			return false;
	...
	/**
	 * Returns true if one SequentialTestCase in a list of
	 * SequentialTestCases equals a specified SequentialTestCase
	...
	public static boolean contains(List<SequentialTestCase> cases,
		...
		for (SequentialTestCase sequentialTestCase : cases) {
			if (equals(stc, sequentialTestCase))
				return true;
		}
		return false;
	}
