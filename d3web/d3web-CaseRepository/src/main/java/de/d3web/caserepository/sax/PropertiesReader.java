...
public class PropertiesReader extends AbstractTagReader {
	...
	protected PropertiesReader(String id) {
		super(id);
	}
		...
		if ("Property".equals(qName)) {
			...
			try {
				...
				Logger.getLogger(this.getClass().getName()).warning(
						"can't handle Property with class " + cn);
	...
	protected void endElement(String uri, String localName, String qName) {
		...
		getCaseObject()
			...
			if (s != null) getCaseObject()
					.getProperties()
						.setProperty(
								Property.CASE_SOURCE_SYSTEM,
								s);
			else Logger.getLogger(this.getClass().getName()).warning(
					"can't handle SourceSystem '" + value + "'");
		}
		else if ("Property".equals(qName) && currentProperty != null) {
			Object value = null;
			String s = getTextBetweenCurrentTag();
			if (currentClass.equals(String.class)) value = s;
			else if (currentClass.equals(Integer.class)) value = Integer.valueOf(s);
			else if (currentClass.equals(Long.class)) value = Long.valueOf(s);
			else if (currentClass.equals(Double.class)) value = Double.valueOf(s);
			else if (currentClass.equals(Boolean.class)) value = Boolean.valueOf(s);

			else if (currentClass.equals(CaseObject.SourceSystem.class)) value = CaseObject.SourceSystem.getForName(s);

			else if (currentClass.equals(PropertiesUtilities.CDataString.class)) value = new PropertiesUtilities.CDataString(
					s);

			if (value != null) getCaseObject().getProperties().setProperty(currentProperty, value);
			currentProperty = null;
		}
