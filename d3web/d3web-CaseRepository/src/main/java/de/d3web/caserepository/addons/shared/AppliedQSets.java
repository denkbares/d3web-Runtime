...
public class AppliedQSets implements IAppliedQSets {
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#containsQSet(de.d3web.kernel
	 * .domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#addQSet(de.d3web.kernel.
	 * domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#removeQSet(de.d3web.kernel
	 * .domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#setEssential(de.d3web.kernel
	 * .domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#resetEssential(de.d3web.
	 * kernel.domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#isEssential(de.d3web.kernel
	 * .domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#addStart(de.d3web.kernel
	 * .domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#resetStart(de.d3web.kernel
	 * .domainModel.qasets.QContainer)
	 */
	 ...
	 * 
	 * @see
	 * de.d3web.caserepository.addons.IAppliedQSets#isStart(de.d3web.kernel.
	 * domainModel.qasets.QContainer)
	 */
	...
	public boolean equals(Object obj) {
		if (obj == null || !(obj instanceof AppliedQSets)) return false;
		...
		if (!a.containsAll(other.a)
				...
				|| !other.s.containsAll(s)) return false;
	 ...
	 * (non-Javadoc)
	 * 
	 * @seede.d3web.caserepository.addons.IAppliedQSets#update(de.d3web.kernel.
	 * domainModel.qasets.Question, java.util.Collection answers)
	 */
	public void update(CaseObject co, Question question) {
		...
		for (TerminologyObject o : question.getParents()) {
			if (o instanceof QContainer) parentQContainers.add((QContainer) o);
		}

		if (answers == null && answers != UndefinedValue.getInstance()) {
			for (QContainer o : parentQContainers) {
				if (!hasAnsweredChildren(co, o)) resetApplied(o);
			}
		}
		else {

			/*
			 * if there is only one parent -> it is activated (remember:
			 * LinkedHashSet won't add an item twice)
			 * 
			 * if there are more parents: if none of these is activated -> there
			 * is no way to discriminate the parents, so they are all activated
			 * if any of these are activated -> no further parent is activated
			 * (heuristic: we treat the question-answer as if it happened in one
			 * of the already activated qcontainers)
			 */

			if (parentQContainers.size() == 1) setApplied((QContainer) parentQContainers.get(0));
			else {
				List<QContainer> activeParents = new LinkedList<QContainer>();
				for (QContainer o : parentQContainers) {
					if (isApplied(o)) activeParents.add(o);
				}
	...
	private boolean hasAnsweredChildren(CaseObject co, QContainer container) {
		for (TerminologyObject o : container.getChildren()) {
			...
			else if (o instanceof QContainer) return hasAnsweredChildren(co, (QContainer) o);
		}
		return false;
	}
