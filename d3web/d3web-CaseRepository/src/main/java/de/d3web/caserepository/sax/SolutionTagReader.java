...
public class SolutionTagReader extends AbstractTagReader {
	...
	private final static Class<? extends PSMethod> DEFAULT_PSMETHOD_CLASS = PSMethodUserSelected.class;
	...
	private void startSolution(Attributes attributes) {
		...
		Solution diag = getKnowledgeBase().searchSolution(id);
		if (diag == null) {
			Logger.getLogger(this.getClass().getName()).warning(
					"no Diagnosis found for " + id + " - omitting");
		...
		if (psMethod != null) { // i.e. it's the new Solution format
			...
			// [MISC]:marty 20031014: legacy code: set a default psmethod, just
			// in case
			// Rationale: if a psmethod occurs in a rating block later,
			// currentSolutionCopy
	...
	private void endSolution() {
		if (currentSolutionCopy == null
				&& currentSolution != null) addSolution(currentSolution);
		currentSolution = null;
	}
	...
	private void startRating(Attributes attributes) {
		...
		if ("D3Classic".equals(psmethodAttr) || "System".equals(psmethodAttr)) psmethodAttr = PSMethodClassicD3.class.getName();
		else if ("de.d3web.Train.kernel.caseImportAddons.PSMethodWebTrain".equals(
				psmethodAttr)
				|| "Author".equals(psmethodAttr)
				|| "WebTrain".equals(psmethodAttr)) psmethodAttr = PSMethodAuthorSelected.class.getName();
		else if ("User".equals(psmethodAttr)) psmethodAttr = PSMethodUserSelected.class.getName();

		try {
			Class<? extends PSMethod> psMethodClass = (Class<? extends PSMethod>) Class.forName(psmethodAttr);
