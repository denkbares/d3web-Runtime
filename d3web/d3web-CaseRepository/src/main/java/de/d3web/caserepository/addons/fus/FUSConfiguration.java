...
public class FUSConfiguration implements IFUSConfiguration {
	...
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null || !(obj instanceof FUSConfiguration))
			return false;
		...
		while (iter.hasNext()) {
			...
			while (oiter.hasNext() && !found) {
				...
				if (!c.getCaseObjectIDProbabilityList().containsAll(oc.getCaseObjectIDProbabilityList())
					|| !oc.getCaseObjectIDProbabilityList().containsAll(c.getCaseObjectIDProbabilityList()))
					continue;
	...
	private boolean equals(AbstractCNode n1, AbstractCNode n2) {
		if (n1 instanceof CNode) {
			if (!(n2 instanceof CNode))
				return false;
			CNode _1 = (CNode) n1;
			CNode _2 = (CNode) n2;
			if (!_1.getType().equals(_2.getType()))
				return false;
			if (_1.getNodes().size() != _2.getNodes().size())
				return false;
		...
		<Case id="fall_1">
			<FUSConfiguration>
				<Configuration>
					<Node type="and">
						<Node type="or">
							<Leaf type="included" diagnosis="P66"/>
							<Leaf type="included" diagnosis="P430"/>
						</Node>
						<Leaf type="excluded" diagnosis="P74"/>
					</Node>
					<FUSs>
						<FUS id="fall_2"/>
						<FUS id="fall_3"/>
	...
	public String getXMLCode() {
		if (set == null || set.isEmpty())
			...
			while (iter.hasNext()) {
				...
				if (conf.getCaseObjectIDProbabilityList() == null || conf.getCaseObjectIDProbabilityList().isEmpty())
					continue;
				else {
