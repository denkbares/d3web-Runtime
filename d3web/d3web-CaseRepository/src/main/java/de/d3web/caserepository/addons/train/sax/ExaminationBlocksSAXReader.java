...
public class ExaminationBlocksSAXReader extends AbstractTagReader {
	...
	private List<IExaminationBlock> blocks = new Vector<IExaminationBlock>();

	protected ExaminationBlocksSAXReader(String id) {
		super(id);
	}

	private static ExaminationBlocksSAXReader instance;

	private ExaminationBlocksSAXReader() {
		this("ExaminationBlocksSAXReader");
	}

	public static AbstractTagReader getInstance() {
		if (instance == null) instance = new ExaminationBlocksSAXReader();
		return instance;
	}
	...
	protected void startElement(String uri, String localName, String qName, Attributes attributes) {
		if (qName.equals("ExaminationBlocks")) startExaminationBlocks();
		else if (qName.equals("ExaminationBlock")) startExaminationBlock(attributes);
		else if (qName.equals("EBTitle"))
		;
		else if (qName.equals("Containers")) startContainers(attributes);
		else if (qName.equals("Container")) startContainer(attributes);
		else if (qName.equals("Solutioncomments"))
		;
		else if (qName.equals("Solutioncomment")) startSolutioncomment(attributes);
	}
	...
	protected void endElement(String uri, String localName, String qName) {
		if (qName.equals("ExaminationBlocks")) endExaminationBlocks();
		else if (qName.equals("ExaminationBlock")) endExaminationBlock();
		else if (qName.equals("EBTitle")) endTitle();
		else if (qName.equals("Containers")) endContainers();
		else if (qName.equals("Container"))
		;
		else if (qName.equals("Solutioncomments"))
		;
		else if (qName.equals("Solutioncomment")) endSolutioncomment();
	}
	...
	private void endExaminationBlocks() {
		...
		ebs = null;
		blocks = new Vector<IExaminationBlock>();
	}
	...
	private List<QContainer> currentContainers = new LinkedList<QContainer>();

	private void startContainers(Attributes attributes) {
		currentContainers = new LinkedList<QContainer>();
	}

	private void endContainers() {
		Iterator<QContainer> iter = currentContainers.iterator();
		while (iter.hasNext())
			currentExBlock.addContent(iter.next());
		currentContainers = new LinkedList<QContainer>();
	}
	...
	private void startContainer(Attributes attributes) {
		...
		if (q == null) {
			Logger.getLogger(this.getClass().getName()).warning(
					"no qcontainer found for id >" + id + "< - omitted");
			return;
		}
	...
	private void startSolutioncomment(Attributes attributes) {
		String id = attributes.getValue("id");
		currentDiagnosis = getKnowledgeBase().searchSolution(id);
	}
