...
public class CaseRepositoryReader {
	...
	private CaseRepository createCaseRepositoryInternal(Object source, KnowledgeBase kb) {
		...
		if (shouldBeReadWithOld) Logger.getLogger(this.getClass().getName()).warning(
				"##########################################\n"
						+
		...
		try {
			...
			if (source instanceof String) parser.parse(new InputSource(new StringReader(
					(String) source)), dh);
			else if (source instanceof File) parser.parse((File) source, dh);
			else if (source instanceof URL) parser.parse(((URL) source).toExternalForm(), dh);
			else if (source instanceof InputStream) parser.parse((InputStream) source, dh);
			...
			Logger.getLogger(this.getClass().getName()).throwing(this.getClass().getName(),
					"createCaseObjectListInternal", x);
		}
