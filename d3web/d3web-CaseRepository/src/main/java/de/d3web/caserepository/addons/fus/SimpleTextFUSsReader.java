...
/**
 * SimpleTextFUSsReader (in ) de.d3web.caserepository.addons.fus
...
public class SimpleTextFUSsReader extends AbstractTagReader {
	...
	protected SimpleTextFUSsReader(String id) {
		super(id);
	}

	private static SimpleTextFUSsReader instance;

	private SimpleTextFUSsReader() {
		this("SimpleTextFUSsReader");
	}

	public static AbstractTagReader getInstance() {
		if (instance == null) instance = new SimpleTextFUSsReader();
		return instance;
	}
	...
	protected void startElement(String uri, String localName, String qName, Attributes attributes) {
		if (qName.equals("SimpleTextFUSs")) startSimpleTextFUSs(attributes);
		else if (qName.equals("SimpleTextFUS")) startSimpleTextFUS(attributes);
	...
	protected void endElement(String uri, String localName, String qName) {
		if (qName.equals("SimpleTextFUSs")) endSimpleTextFUSs();
		else if (qName.equals("SimpleTextFUS")) endSimpleTextFUS();
		else if (qName.equals("Name")) endName();
		else if (qName.equals("Text")) endText();
	}
	...
	private void endName() {
		if (stfs != null) name = XMLTools.prepareFromCDATA(getTextBetweenCurrentTag());
	}

	private void endText() {
		if (stfs != null) text = XMLTools.prepareFromCDATA(getTextBetweenCurrentTag());
	}
