...
import de.d3web.caserepository.addons.fus.internal.ProbabilityList;
...
public class FUSConfigurationReader extends AbstractTagReader {
	...
	protected FUSConfigurationReader(String id) {
		super(id);
	}

	private static FUSConfigurationReader instance;

	private FUSConfigurationReader() {
		this("FUSConfigurationReader");
	}

	public static AbstractTagReader getInstance() {
		if (instance == null) instance = new FUSConfigurationReader();
		return instance;
	}
	...
	public void initialize(KnowledgeBase knowledgeBase, CaseObjectImpl caseObject) {
		openNodes = new LinkedList<CNode>();
		...
		if (CNode.AND.getName().equals(typeS)) {
			...
			try {
				...
				Logger.getLogger(this.getClass().getName()).warning(
						"CNode.NOFM can't be set with n='" + attributes.getValue("n") + "'");
				return;
			}
		}
		else {
			Logger.getLogger(this.getClass().getName()).warning(
					"no way to handle CNodes of type '" + type + "'");
			return;
		}
	...
	private void startLeaf(Attributes attributes) {
		...
		if (openNodes.isEmpty()) currentConf.setNode(l);
		else (openNodes.get(0)).addNode(l);

	}

	private void endNode() {
		if (openNodes.size() == 1) currentConf.setNode(openNodes.get(0));
		CNode c = openNodes.remove(0);
		(openNodes.get(0)).addNode(c);
	}
