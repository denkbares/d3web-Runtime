...
/**
 * A new {@link SearchAlgorithm} that delegates the search to a sequence of search algorithms and apply each search
 * function on the same search model until the optimal path has been found.
 * <p>
 * To optimize searching, each search function should ensure that is will stop searching if no better path could be
 * found.
 *
 * @author volker_belli
 * @created 01.09.2011
...
public class MultiSearchAlgorithm implements SearchAlgorithm {
	...
	public AbortStrategy getAbortStrategy() {
		for (SearchAlgorithm delegate : delegates) {
