...
/**
 * QContainer Node for the virtual graph. Provides easy access to cost-benefit issues of QContainers.
 *
...
public class Node {
	...
	public Node(QContainer qcon, SearchModel cbm) {
		...
		this.st = StateTransition.getStateTransition(qContainer);
	}

	/**
	 * Checks if the Node is applicable in session. A node is applicable if the preconditions of the associated
	 * QContainer are matching session.
	 */
	...
	/**
	 * Ensures that all questions of the Node's QContainer are answered. For unanswered Questions the expected values
	 * are set.
	 *
